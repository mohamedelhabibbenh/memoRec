...
export default function Comp_memoFavorites(props) {
...
`;
  return (<Header>
    ...
    <div>
      ...
      <Link to='/my_favs' className="text-link" style={{ color: cols.orangy_red }} >My Favorites</Link>
...
`;
  ...
  return (<Content >
    <Header>
      <p style={{ color: cols.white }}>memoRec<span style={{ color: cols.black }}>/My Favorites</span></p>
      <h2>My Favorites</h2>
    </Header>
    <MemoListContainer>
      {memoList.lenght > 0 ? <h5>Nothing is here :)</h5> : <></>}
      {memoList.map((m, indx) => {
        return (m.favorite&&!m.removed)
        ?(<Cmp_memo key={indx}
