...
export default function CreateMemo(props) {
    const [tmpMemo ,setTmpMemo] = useState({title:"",favorite:false, removed:false,content:'',color:cols.unclear_white})

    const { fn_close} = props;
    const { fn_createMemo} = props.methods;
    const handleFav=()=>{
        let k = {...tmpMemo};
